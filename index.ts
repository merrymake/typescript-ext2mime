export class MimeType<T extends string, E extends string> {
  constructor(private type_MimeType: T, private tail_MimeType: E) {}
  public toString() {
...
export function allMimeTypesOf(ext: string): MimeType<string, string>[] {
  let res = (
    COMMON_MIME_TYPES as { [key: string]: MimeType<string, string>[] }
  )[ext.toLowerCase()];
  return res === undefined ? [] : res;
}
export function optimisticMimeTypeOf(
  ext: string
): MimeType<string, string> | null {
  let res = allMimeTypesOf(ext);
  return res.length === 0 ? null : res[0];
}
export function optimisticMimeTypeDefault<T>(
  ext: string,
  def?: T
): MimeType<string, string> | T | undefined {
  let res = allMimeTypesOf(ext);
